...
from src.datasets import AspectCategorizationDataset
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune BERT model for aspect categorization task.")
    ...
    train_dataset = AspectCategorizationDataset(
        "./data/raw/SentiRuEval_rest_markup_train.xml", tokenizer=tokenizer,
        label_to_id=LABEL_TO_ID,
    )
    test_dataset = AspectCategorizationDataset(
    ...
    trainer = Trainer(
        model=model,
        main_metric=F1Score(len(train_dataset.label_to_id)),
        optimizer=optimizer,
        ...
        metrics=[
            RecallScore(len(train_dataset.label_to_id)),
            PrecisionScore(len(train_dataset.label_to_id)),
        ],

...
from src.datasets import TABSADataset
from src.metrics import RecallScore
from src.metrics import F1Score
from src.metrics import PrecisionScore
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune BERT model for TABSA task.")
    ...
    parser.add_argument("--aux_construct_mode", type=str, default="QA-M", help="Choose either `QA-M` or `NLI-M`")
    ...
    N_LABELS = 4
    aspects = {"Food", "Interior", "Price", "Service", "Whole"}
    train_dataset = TABSADataset(
        "./data/raw/SentiRuEval_rest_markup_train.xml", tokenizer=tokenizer,
        mode=args.aux_construct_mode,
        label_to_id=LABEL_TO_ID,
        aspects=aspects,
        none_label=None,
    )
    CLASS_WEIGHTS = train_dataset.class_weights
    logger.info("Class weights are: %s" % {label:CLASS_WEIGHTS[id_] for label, id_ in LABEL_TO_ID.items()})
    test_dataset = TABSADataset(
    ...
    LOSS = torch.nn.CrossEntropyLoss(weight=torch.as_tensor(CLASS_WEIGHTS, dtype=torch.float32))
    # tensorboard_writer = SummaryWriter(log_dir="logs/fit")
    trainer = Trainer(
        model=model,
        main_metric=F1Score(N_LABELS),
        optimizer=optimizer,
        ...
        metrics=[
            RecallScore(N_LABELS),
            PrecisionScore(N_LABELS),
            ConfusionMatrix(N_LABELS),
        ],
